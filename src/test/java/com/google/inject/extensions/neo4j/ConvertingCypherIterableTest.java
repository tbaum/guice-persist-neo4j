...
public class ConvertingCypherIterableTest {
    ...
    public void testNoResult() {
        Iterable<Node> r = cypher.execute("START n=node({ref}) MATCH n-[:NOTHING]->r RETURN r",
    ...
    public void testMultipleResults() {
        Iterable<Node> r = cypher.execute("START n=node({ref}) MATCH n-[:REL]->r RETURN r",
                map("ref", ref), (f) -> f.get("r"));
    ...
    public void testSingleResult() {
        Iterable<Node> r = cypher.execute("START n=node({ref}) MATCH n-[:REL]->r WHERE r.name='n1' RETURN r",
                map("ref", ref), (f) -> f.get("r"));
    ...
    public void testSingleResult1() {
        Node r = cypher.singleResult("START n=node({ref}) MATCH n-[:REL]->r WHERE r.name='n1' RETURN r",
                map("ref", ref), (f) -> f.get("r"));
    ...
    public void testSingleResult2() {
        Node r = cypher.singleResult("START n=node({ref}) MATCH n-[:NOTHING]->r RETURN r",
                map("ref", ref), (f) -> f.get("r"));
        assertNull(r);
    }
